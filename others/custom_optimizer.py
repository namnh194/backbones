"""
    Refer: https://github.com/jankrepl/mildlyoverfitted/blob/master/mini_tutorials/custom_optimizer_in_pytorch/src.py
"""
...
if __name__ == "__main__":
    ...
    path_adam = run_optimization(xy_init, Adam, n_iter, lr=1e-3)
    path_sgd = run_optimization(xy_init, SGD, n_iter, lr=1e-3)
    path_adagrad = run_optimization(xy_init, Adagrad, n_iter, lr=1e-3)
    
    freq = 10
    paths = [path_adam[::freq], path_sgd[::freq], path_adagrad[::freq]]
    colors = ["green", "blue", "yellow"]
    names = ["Adam", "SGD", "Adagrad"]
    anim = create_animation(paths, colors, names, figsize=(12, 7), x_lim=(-.1, 1.1), y_lim=(-.1, 1.1), n_seconds=7)
